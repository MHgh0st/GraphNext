  ...
  return (
    <div
      className={`
        ...
        ${isSelected
        ? "bg-blue-50/90 border-blue-400 shadow-md shadow-blue-500/10 ring-1 ring-blue-400/20"
        : "bg-white/60 border-slate-100 hover:border-blue-300 hover:bg-white/90 hover:shadow-sm"
      }
      `}
...
export const NodeTooltip = ({
  ...
  // 🟢 محاسبه مجموع کل پرونده‌های ورودی به گره با لایه ایمن
  const totalIncomingCases = useMemo(() => {
    return incomingEdges.reduce((sum, item) => sum + (item.caseCount || 0), 0);
  }, [incomingEdges]);

  // 🟢 محاسبه مجموع کل پرونده‌های خروجی از گره با لایه ایمن
  const totalOutgoingCases = useMemo(() => {
    return outgoingEdges.reduce((sum, item) => sum + (item.caseCount || 0), 0);
  }, [outgoingEdges]);
  ...
  return (
    <>
      ...
      {/* 🟢 بخش جدید: نمایش بنتوکارت‌های آماری حجم ورودی/خروجی گره فعال */}
      {nodeTooltipData.length > 0 && (
        <div className="grid grid-cols-2 gap-2 px-3 py-2 border-b border-slate-100 bg-white/40 backdrop-blur-sm">
          <div className="flex items-center gap-2 p-2 bg-emerald-50/60 rounded-xl border border-emerald-100/40">
            <div className="p-1.5 bg-white rounded-lg text-emerald-600 shadow-sm">
              <LogIn size={12} />
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-[9px] text-emerald-600/80 font-bold">کل ورودی‌ها</span>
              <span className="text-xs font-black text-emerald-700 font-vazir">
                {totalIncomingCases.toLocaleString("fa-IR")}
              </span>
            </div>
          </div>

          <div className="flex items-center gap-2 p-2 bg-rose-50/60 rounded-xl border border-rose-100/40">
            <div className="p-1.5 bg-white rounded-lg text-rose-600 shadow-sm">
              <LogOut size={12} />
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-[9px] text-rose-600/80 font-bold">کل خروجی‌ها</span>
              <span className="text-xs font-black text-rose-700 font-vazir">
                {totalOutgoingCases.toLocaleString("fa-IR")}
              </span>
            </div>
          </div>
        </div>
      ...
      <CardBody className="p-0 overflow-hidden bg-slate-50/50">
        {nodeTooltipData.length === 0 ? (
          ...
          <ScrollShadow className="h-full max-h-[1000px] p-3 scrollbar-hide">
            <Accordion
